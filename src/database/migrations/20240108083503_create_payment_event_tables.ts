...
export async function up(knex: Knex): Promise<void> {
  ...
  CREATE TABLE fill_event_payment_event (
		payment_event_id INTEGER REFERENCES payment_event (id),
	...
	CREATE TABLE stripe_payment_intent (
		...
		payment_event_id INTEGER REFERENCES payment_event (id) NOT NULL,
  ...
  CREATE TABLE stripe_dispute (
		id INTEGER AUTO_INCREMENT PRIMARY KEY,
		stripe_payment_intent_id INTEGER REFERENCES stripe_payment_intent (id) NOT NULL,
		status VARCHAR(255),
		reason VARCHAR(255),
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
